...
<aui:form action="<%= forgotPasswordURL %>" method="post" name="fm">
	...
	<liferay-ui:error exception="<%= UserLockoutException.PasswordPolicyLockout.class %>">
		...
		<liferay-ui:message arguments="<%= ule.user.getUnlockDate() %>" key="Your Account is Locked. Please Email to portaladmin@socialsolutions.biz to Get Your Account Unlocked. " translateArguments="<%= false %>" />
	</liferay-ui:error>
			...
			<c:when test="<%= (user2 != null) && Validator.isNotNull(user2.getEmailAddress()) %>">
				...
				<c:if test="<%= Validator.isNotNull(user2.getReminderQueryQuestion()) && Validator.isNotNull(user2.getReminderQueryAnswer()) %>">
					...
					<div class="alert alert-info">
						<liferay-ui:message arguments="<%= login %>" key="Password reset link will be sent to your email address if you can correctly answer the secret question" translateArguments="<%= false %>" />
					</div>
